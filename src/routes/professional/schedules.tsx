...
import { useMyProfile } from "@/hooks/useProfessionals";
import { useWorkSchedules, useReplaceWorkSchedules } from "@/hooks/useWorkSchedules";
...
function fromApiData(apiData: WorkScheduleDTO[]): DayEntry[] {
...
function ScheduleEditor({ professionalId, initialEntries }: ScheduleEditorProps) {
  const [entries, setEntries] = useState<DayEntry[]>(initialEntries);
  const mutation = useReplaceWorkSchedules(professionalId);
  ...
  return (
    <>
      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted border border-salmon-100 w-fit">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 w-28 shrink-0">
                  <button
                    ...
                    onClick={() => updateEntry(entry.weekday, { is_active: !entry.is_active })}
                    className={cn(
...
function Schedules() {
  ...
  const { data: scheduleData, isLoading: scheduleLoading } = useWorkSchedules(professionalId);
    ...
    <main className="flex-1 p-4 md:p-8 pb-28">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Defina sua disponibilidade semanal</p>
        </div>
        ...
        {scheduleLoading ? (
          ...
          <ScheduleEditor
            key={scheduleData?.map((s) => s.id).join(",") ?? "empty"}
            professionalId={professionalId}
            initialEntries={fromApiData(scheduleData ?? [])}
          />
        )}
      </div>
    </main>
